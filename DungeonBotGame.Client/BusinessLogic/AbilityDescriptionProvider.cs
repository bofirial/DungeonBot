...
namespace DungeonBotGame.Client.BusinessLogic
{
    ...
    public class AbilityDescriptionProvider : IAbilityDescriptionProvider
    {
        private readonly Dictionary<AbilityType, AbilityDescriptionViewModel> _abilityDescriptions = new()
        {
            ...
            { AbilityType.SurpriseAttack, new AbilityDescriptionViewModel("Surprise Attack", "The first action in every encounter takes place immediately and deals double damage.  If the action targets an enemy, the enemy is stunned for 2 seconds.", AbilityType.SurpriseAttack, IsTargettedAbility: false, CooldownCombatTime: 0) },
